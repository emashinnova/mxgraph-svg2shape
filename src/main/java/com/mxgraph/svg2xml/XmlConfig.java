/**
 * $Id: XmlConfig.java,v 1.6 2013/04/22 05:32:08 mate Exp $ Copyright (c) 2013,
 * JGraph
 */
...
{
    // NOTE: don't set these variables manually, they are read from the XML
    // config files.
    ...
    // use relative scaling (used if you want to change the size of resulting
    // XMLs)
    ...
    // every stencil will be absoluteScalingX in X size, if
    // relativeScaling=false
    private double absoluteScalingX = 100d;

    // every stencil will be absoluteScalingY in Y size, if
    // relativeScaling=false
    ...
    // NOTE: do not set groupName here, since it is read from the groups config
    // file. add an entry such as "groupname: yourgroupname;" without the
    // quotations to your groups XML (config file)
    // if null, group name is generated from Svg2xml.stencilUserMarker and
    // folder structure
    ...
    private void setRelativeScalingRatio(double relativeScalingRatio,
        Svg2XmlGui gui)
    {
    ...
    {
        String strokeFromStyle = Optional.ofNullable(getStyle())
            .map(s -> s.getStrokeWidth())
            .filter(w -> w != null && !"".equals(w)).orElse(null);

        return strokeFromStyle != null ? strokeFromStyle : strokewidth;
    }
